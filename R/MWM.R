...
#' MWM
#'
#' This function defines a MWM  module for incorporation into a
#' psychTestR timeline.
#' Use this function if you want to include the MWM in a
#' battery of other tests, or if you want to add custom psychTestR
#' pages to your test timeline.
#'
#' For demoing the MWM, consider using \code{\link{MWM_demo}()}.
#' For a standalone implementation of the MWM,
#' consider using \code{\link{MWM_standalone}()}.
...
#' @param label (Character scalar) Label to give the MWM results in the output file.
#' @param feedback (Function) Defines the feedback to give the participant
#' at the end of the test.
#' @param dict The psychTestR dictionary used for internationalisation.
#' @param adaptive (Scalar boolean) Indicates whether you want to use the adaptive MWM2 (TRUE)
#' or the non-adaptive MWM (FASLE). Default is adaptive = TRUE.
...
MWM <- function(num_items = 18L,
                with_welcome = TRUE,
                take_training = FALSE,
                with_finish = TRUE,
                label = "MWM",
                feedback = MWM_feedback_with_score(),
                dict = MWM::MWM_dict,
                autoplay = TRUE,
  ...
  audio_dir <- "https://s3.eu-west-1.amazonaws.com/media.dots.org/stimuli/MWM"
  take_training <- FALSE
  ...
  psychTestR::join(
    ...
    if (take_training) psychTestR::new_timeline(instructions(audio_dir),
                                                dict = dict),
    if (with_welcome) MWM_welcome_page(),
    ...
    feedback,
    if(with_finish) MWM_finished_page(),
